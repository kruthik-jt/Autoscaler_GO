...
func currentMetrics(m [][10]measurementPod, measurementIndex int32, pMeasurementindex int32) float64 {
	...
	//Current Metrics in Precentage = (Average CPU Utlization in Milicore) / Request Value * 100 = Total CPU (core) / (Number of pods * request Value)
	//fmt.Println("Total CPU: ", totalCpu, "\nTotal number of pods: ", numPods)
	currMetrics = totalCpu * baseMilicore * 100.0 / (requestValue * float64(numPods))
	//Printing and returning the current metrics in precentage.
	//fmt.Println("Current Metrics = ", currMetrics)
	return currMetrics
}
...
func main() {
	...
	var measurementIndex int32 = 0      //Meaurement Index stats with 0 until 20000
	...
	for {
		...
		if iter == 30 {
			...
			replicaCount, preDecision = scalingAlgorithm(replicaCount, measurements[:][:], measurementIndex, pMeasurementindex, preDecision)
